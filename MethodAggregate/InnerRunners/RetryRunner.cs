        ...
        private static void Retry(ref int retryCounts, TimeSpan interval, Action action, RunnerOption option)
        {
            try
            {
                ...
                else
                {
                    if (option.MailOptionConstructor != null)
                    {
                        RunnerMailSender.Send(option.MailOptionConstructor(new RunnerMailOptionContext(MaxRetryCountFailedException(option.RetryCount))));
                    }
        ...
        private static void Retry<T>(ref int retryCounts, TimeSpan interval, Action<T> action, T request, RunnerOption<T> option)
        {
            try
            {
                ...
                else
                {
                    if (option.MailOptionConstructor != null)
                    {
                        RunnerMailSender.Send(option.MailOptionConstructor(new RunnerMailOptionContext<T>(request, true, MaxRetryCountFailedException(option.RetryCount))));
                    }
        ...
        private static T Retry<T>(ref int retryCounts, TimeSpan interval, Func<T> func, RunnerOption<T> option)
        {
            ...
            try
            {
                ...
                else
                {
                    if (option.MailOptionConstructor != null)
                    {
                        RunnerMailSender.Send(option.MailOptionConstructor(new RunnerMailOptionContext<T>(response, false, MaxRetryCountFailedException(option.RetryCount))));
                    }
        ...
        private static TResponse Retry<TRequest, TResponse>(ref int retryCounts, TimeSpan interval, Func<TRequest, TResponse> func, TRequest request, RunnerOption<TRequest, TResponse> option)
        {
            ...
            try
            {
                ...
                else
                {
                    if (option.MailOptionConstructor != null)
                    {
                        RunnerMailSender.Send(option.MailOptionConstructor(new RunnerMailOptionContext<TRequest, TResponse>(request, response, MaxRetryCountFailedException(option.RetryCount))));
                    }
