...
{
    public class NeweggAPIMailer : IRunnerMailer
    {
        public bool Send(RunnerMailContent content)
        {
            return Send(content, false);
        }

        private bool Send(RunnerMailContent option, bool isNeedLog = false)
        {
            return option == null ? false : Send(option.From, option.To, option.CC, option.Subject, option.Body, (MailContentType)Enum.Parse(typeof(MailContentType), option.ContentType.ToString()), (MailPriority)Enum.Parse(typeof(MailPriority), option.Priority.ToString()), isNeedLog);
        }

        private bool Send(string from, string to, string cc, string subject, string body, MailContentType contentType = MailContentType.Html, MailPriority priority = MailPriority.Low, bool isNeedLog = false)
        {
            ...
            finally
            {
                ...
                {
                    WriteEmailLog(from, to, cc, subject, body, isSuccessful, errorMsg);
                }
            }
        }

        private void WriteEmailLog(string from, string to, string cc, string subject, string body, bool isSuccessful, string errorMsg)
        {
